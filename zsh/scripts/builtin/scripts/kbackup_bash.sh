...
function kbackup:usage() {
  echo "----------------------------"
  echo "$0 backs up kubernetes resource definitions in yaml"
  echo ""
  echo "Usage: "
  echo "$0 --dir base_dir [other_options]"
  echo ""
...
if ! type "kubectl" > /dev/null; then
  ...
  kb_log "kubectl found: $(type kubectl)"
fi
...
if [ -z "$namespaces_input" ]; then
  ...
  namespaces=($(kubectl get namespaces | awk 'NR>1 {print $1}' | xargs))
else
  kb_log "--namespaces supplied: $namespaces_input"
  IFS=',' read -r -a namespaces <<< "$namespaces_input"
fi
...
if [ -z "$resources_input" ]; then
  ...
  resource_types=($(kubectl "api-resources" | awk 'NR>1 {print $1}' | xargs))
else
  kb_log "--resources supplied: $resources_input"
  IFS=',' read -r -a resource_types <<< "$resources_input"
fi
